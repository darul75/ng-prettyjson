...
a.isObject(b)&&e(b.json)?(l(),f.$watch(m+".json",function(a){n(a),i=a},!0)):(n(b),i=b),j&&j.setValue(b)},!0);var o=function(){try{i=JSON.parse(j.getValue()),f.parsable=!0}catch(a){f.parsable=!1}// trigger update
f.$apply(function(){})};f.edit=function(){return f.aceEditor?(f.editActivated?(j&&(document.getElementById(f.id).env=null),n(i)):(j=ace.edit(f.id),j.setOptions({maxLines:1/0}),j.on("change",o),j.getSession().setMode("ace/mode/json")),void(f.editActivated=!f.editActivated)):void(console&&console.log("'ace lib is missing'"))},f.update=function(){f.$emit("json-updated",i),f.onEdit&&f.onEdit({newJson:i}),this.edit()}}}}]).factory("ngPrettyJsonFunctions",function(){// cache some regular expressions
